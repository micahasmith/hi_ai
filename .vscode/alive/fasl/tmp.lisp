...
(defun %bind-task-intent-trace (task-id facts)
  "Run the intent pipeline and return the raw capture-> trace."
  (hi:capture+tap-> (list :task-id task-id :facts facts)
...
(defun bind-task-intent (task-id facts)
  ...
  (let ((trace (%bind-task-intent-trace task-id facts)))
    (append facts (cdr (assoc :task-facts trace)))))
